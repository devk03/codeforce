...
```bash
make              # Default: compile, run with input.txt, then clean up executable
make run          # Same as default (compile, run, clean)
make compile      # Compile only (creates 'main' executable)
make clean        # Remove the executable
make new          # Save current solution and reset from template
```

### Testing

- Add test cases to `input.txt`
- `make` or `make run` automatically uses `input.txt` as stdin and cleans up the executable after running
...
- The Makefile redirects `input.txt` to stdin using `< input.txt`
